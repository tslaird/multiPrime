  ...
  if [ -d "$dir" ]; then
    ...
    if [ -f "$ALL_FA" ] && [ -f "$YAML_FILE" ]; then
      ...
      if [ ! -f "$SENTINEL_FILE" ] || [ "$ALL_FA" -nt "$SENTINEL_FILE" ]; then
        ...
        ../../scripts/hash_fasta.py -i $ALL_FA -o ${ALL_FA%.*##*/}.hash.fa -m ${ALL_FA%.*##*/}.hashmap
        ...
        snakemake --configfile $YAML_FILE -s ../../multiPrime.py --cores 4 --resources mem_mb=60000 -C input_dir=$FULL_DIR results_dir=$FULL_DIR/results log_dir=$FULL_DIR/logs virus=${ALL_FA%.*##*/}.hash && touch .snakemake.done
        if [ -f "results/Core_primers_set/BWT_coverage/core_final_maxprimers_set.out" ] && [ -f ".snakemake.done" ]; then
            # extract amplicons
            ../../scripts/extractAmplicon.py -t results/Core_primers_set/BWT_coverage/core_final_maxprimers_set.out -f ${ALL_FA%.*##*/}.hash.fa -o ${ALL_FA%.*##*/}_ampicons_hash.fasta
            # map back to original headers
            ../../scripts/map_hash_fasta.py -m ${ALL_FA%.*##*/}.hashmap -i ${ALL_FA%.*##*/}_ampicons_hash.fasta -o ${ALL_FA%.*##*/}_ampicons.fasta
            # make final output table
            ../../scripts/build_table.py -t results/Core_primers_set/BWT_coverage/core_final_maxprimers_set.out -p results/Core_primers_set/core_final_maxprimers_set.fa -g results/Total_fa/${ALL_FA%.*##*/}.hash.format.fa -o ${ALL_FA%.*##*/}_primer_amplicon_table.csv -m ${ALL_FA%.*##*/}.hashmap
